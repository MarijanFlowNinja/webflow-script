  ...
  function validateField(cfg) {
    ...
    switch (cfg.type) {
      ...
      case "email":
        ...
        break;
      case "phone":
        // očistimo sve osim + i cifara
        const normalized = v.replace(/[^\d+]/g, "");
        const digits = normalized.replace(/\D/g, "");
        if (!normalized) {
          err = "required";
        } else if (digits.length < cfg.min) {
          err = "min";
