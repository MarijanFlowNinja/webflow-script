(function () {
  ...
  function populateUtmFields(form) {
    const map = {
      ...
      campid:           'utmCampid1', utm_campid:     'utmCampid1',
      gclid:            'gCLID1',
    ...
    Object.entries(map).forEach(([param,inputName]) => {
      const val = urlParams.get(param) || cookieParams.get(param) || '';
  ...
  async function populateAll(form) {
    ...
    document.querySelectorAll('input[name="domo_id"]').forEach(i => i.value = getOrCreateDomoID());
    document.querySelectorAll('input[name="g_id"]').forEach(i => i.value = getGaClientId());
    await (async function fillFFID() {
    ...
    fetch('https://api.ipify.org?format=json')
      ...
      .then(data => {
        ...
        document.querySelectorAll('input[name="geoip_country_code"]').forEach(i => i.value = iso);
        if (iso === 'CA') {
  ...
  function populateSubmissionFields(form) {
    ...
    if (tfi) {
      const ts = new Date();
      tfi.value = `${String(ts.getMonth()+1).padStart(2,'0')}/${String(ts.getDate()).padStart(2,'0')}/${ts.getFullYear()} ${String(ts.getHours()).padStart(2,'0')}:${String(ts.getMinutes()).padStart(2,'0')}:${String(ts.getSeconds()).padStart(2,'0')}`;
    }
    ...
    const codEl = form.querySelector('input[name="sFDCCanadaEmailOptInOutDate1"]');
  ...
  const VALIDATION_RULES = {
    first_name: { type:'name', min:2, required:true, messages:{ required:'First name is a required field.', min:'Please enter two or more characters.', invalid:'Please enter a valid first name.' }},
    last_name : { type:'name', min:2, required:true, messages:{ required:'Last name is a required field.', min:'Please enter two or more characters.', invalid:'Please enter a valid last name.' }},
    email     : { type:'email', required:true, messages:{ required:'Email is a required field.', invalid:'Please make sure the email address is formatted as name@domain.com.', business:'Please enter a valid business email address. Personal emails such as Gmail are not accepted.' }},
    phone     : { type:'phone', min:10, required:true, messages:{ required:'Phone number is a required field.', min:'Please enter a minimum of 10 digits.', invalid:'Please enter a valid phone number.' }},
    title     : { type:'title', required:true, messages:{ required:'Job title is required.', invalid:'Please enter a valid job title.' }}
  };
  ...
  function initContactUsDynamic(form) {
    ...
    sub.addEventListener('change', updateFields);
  ...
  async function handleSubmit(e) {
    ...
    form.querySelectorAll('select[required]').forEach(sel => { if (!validateSelect(sel)) valid = false; });
    if (!valid) return console.log('Please fix errors and try again.');

    await populateAll(form);
    populateSubmissionFields(form);
    const elqVal = form.querySelector('input[name="elqFormName"]')?.value;
    if (elqVal === 'website_cta_videodemorequest') setUInfoCookie(form);
    ...
    .then(res => {
      ...
      window.location.href = redirectInput ? redirectInput.value : window.location.origin;
    })
